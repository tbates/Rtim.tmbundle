# Fork of Markdown.tmbundle Support/lib/markdown.rb
# Fixed for Ruby 1.9+: String#to_a was removed; use String#lines instead.
# Used by Rtim overrides of New Item / New Subitem (same UUIDs as managed Markdown bundle).
...
module Markdown
	...
	class List
		...
		def List.parse(str, line = 0)
			...
			# Ruby 1.8 used str.to_a for line array; Ruby 1.9+ uses lines
			first_line = str.to_s.lines[0].to_s
			list.numbered = /^\s*([0-9])/.match(first_line)
			list.unordered_list_mark = '-' if /^\s*-/.match(first_line)
			list.line = line
			...
			linenumber = -1
			begin
				# Recomputed on SubList retry (str may have been advanced)
				lines = str.to_s.lines
